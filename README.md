...
> **模式自动恢复（v1.13.15 起）**：考试系统"开始考试"多为整页跳转，跳转/刷新后插件会自动恢复之前开启的模式（同域名内有效，跨站点不恢复、file:// 本地页不恢复），无需重新按快捷键。
...
| `Ctrl+↑` / `Ctrl+↓` | 调答题速度 | 后台模式题间延时 -1s / +1s（1-60 秒），运行中即时生效，右下角轻提示 |
...
**匹配引擎（v1.13.20 优化）**：相似度双上界预筛（长度差 + bigram 公共度，低于收集线直接跳过精确计算）+ 题库 3-gram 倒排索引取 top800 候选——万级题库从"页面无响应 3 分钟"降到约 15 秒，预筛为数学严格上界，不误杀真匹配。
...
| DOM 签名 | 注入时生成随机前缀 |
| 快捷键 | 非标准组合键 |
| 跨站隔离 | 模式状态带站点校验，A 站开启不会在 B 站表单页自动作答 |
